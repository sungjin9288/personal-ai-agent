...
| R4 Operator UI module boundary | 진행 중 | harness browse, mission/action inbox, release lifecycle action을 explicit dependency 모듈로 이동 |
...
| R4.3.2 Release navigation actions | 다음 작업 | history 이동, copy, preview action dispatch를 별도 경계로 정리 |
...
- release 세부 경계:
  ...
  - R4.3.2 navigation action: release history 이동, copy, preview action dispatch
