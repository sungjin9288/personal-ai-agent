...
| R4 Operator UI module boundary | 진행 중 | harness browse, mission/action inbox, release lifecycle·navigation action을 explicit dependency 모듈로 이동 |
...
| R4.3.2b Release copy actions | 다음 작업 | copy action 71개의 routing을 복사 대상별로 정리 |
| R4.3.2c Release preview actions | 예정 | preview action 2개를 분리하고 release surface의 generic listener를 제거 |
...
- release 세부 경계:
  ...
  - R4.3.3 render assembly: 상태 요약과 세부 card를 읽는 흐름에 맞춰 하위 surface로 분리
