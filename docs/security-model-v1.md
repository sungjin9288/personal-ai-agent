...
- [target-identity-session-operations-v1.md](target-identity-session-operations-v1.md) defines the target identity/session operations evidence contract for customer IdP onboarding, user lifecycle, session lifecycle, role administration, permission propagation, audit export, break-glass, support impersonation, compliance, retention, and the remaining target identity evidence gap
