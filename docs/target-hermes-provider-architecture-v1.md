...
The current approved path remains OpenAI-scoped self-hosted/local-first pilot. Hermes provider readiness remains blocked until this architecture decision is approved and endpoint ownership, model pinning, target secret injection, tool-call parsing, session lifecycle, transcript policy, quota guard, telemetry, fallback, customer approval, and Hermes live validation evidence are generated from the approved production-like or hosted target environment.
