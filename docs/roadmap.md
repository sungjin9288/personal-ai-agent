...
- release current open blocker triage now also provides a slice-level package copy action, so the currently filtered summary, handoff, closure checklist, command runbook, and evidence bundle can be copied together when one complete operator handoff package is needed
- release current open blocker triage now also provides a slice-level closure checklist copy action, so the currently filtered category/owner blocker set can be handed off with per-blocker closing evidence, evidence docs, commands, artifact refresh guidance, and final verification gates
...
- release current open blocker rows and focused callouts now provide single-blocker package copy actions, so one stop-condition can be handed off with metadata, closure requirements, commands, and evidence docs without copying the active slice package
- release current open blocker rows and focused callouts now provide single-blocker closure checklist copy actions, so one stop-condition can be handed off with required closing evidence, evidence docs, commands, artifact refresh guidance, and final verification gates
...
- target local provider architecture remains blocked until endpoint ownership proof, LOCAL_PROVIDER_MODEL model pinning proof, network isolation proof, secret and credential policy proof, runtime lifecycle proof, session and artifact provenance proof, data residency and transcript policy proof, quota and resource guard proof, telemetry proof, fallback and customer approval proof, target-boundary npm run live:execution-v1:local pass, release artifact hygiene result, and regenerated execution snapshot evidence are recorded, while Hermes provider validation remains blocked until target Hermes provider architecture evidence for endpoint ownership proof, HERMES_PROVIDER_MODEL model pinning proof, target secret injection proof, tool-call parsing proof, session lifecycle proof, data and transcript policy proof, quota and rate guard proof, telemetry proof, fallback and stop-condition proof, customer approval proof, target-boundary npm run live:execution-v1:hermes pass, release artifact hygiene result, and regenerated execution snapshot is recorded
...
- attach target local provider architecture evidence for endpoint ownership proof, LOCAL_PROVIDER_MODEL model pinning proof, network isolation proof, secret and credential policy proof, runtime lifecycle proof, session and artifact provenance proof, data residency and transcript policy proof, quota and resource guard proof, telemetry proof, fallback and customer approval proof, target-boundary local live validation, release artifact hygiene, and regenerated execution snapshot; attach target Hermes provider architecture evidence for endpoint ownership proof, HERMES_PROVIDER_MODEL model pinning proof, target secret injection proof, tool-call parsing proof, session lifecycle proof, data and transcript policy proof, quota and rate guard proof, telemetry proof, fallback and stop-condition proof, customer approval proof, target-boundary Hermes live validation, release artifact hygiene, and regenerated execution snapshot; rerun live validation; and archive updated evidence
