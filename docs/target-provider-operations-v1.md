...
| Provider fallback runtime audit | mission fallback policy, mission timeline, workspace timeline, operator timeline, provider events, and provider attention remediation smokes pass locally | explicit failover attempt, selected fallback provider, non-provider-failure stop condition, provider event family, operator chronology, attention remediation command, and customer impact decision are archived from the target boundary |
...
- provider fallback runtime audit proof with `mission run --fallback-provider`, `mission timeline`, `workspace timeline`, `overview operator-timeline`, `provider events --family fallback`, and `action remediate-provider-attention --fallback-provider` evidence for provider-failure-only failover, selected fallback provider, and deterministic stop conditions
