...
- lastFullSweep: 286/286 passed with `npm run smoke:all` on 2026-08-03
...
| Q10 evidence-gated answer shadow | `npm run smoke:evidence-gated-answer-shadow` | Verifies Q9-authoritative five-state routing, four pre-generation exits, one fake generator call, Q7/Q1 binding, content-free artifacts, and simple plus rehashed tamper rejection |
| Q11 multi-scenario robustness | `npm run smoke:evidence-gated-answer-robustness` | Verifies the frozen 12-row structural matrix, four sufficient-only deterministic calls, Q7/Q9/Q10 byte binding, content-free output, and semantic tamper rejection; it does not assert multilingual semantic quality |
| Q12 local artifact writer hardening | `npm run smoke:evidence-gated-answer-output-hardening` | Verifies static and dynamic link/path swaps, partial and failed writes, crash recovery, complete JSON, orphan recovery, concurrency, exact bytes, mode `0600`, and one link; it cannot close Node v24's final same-user syscall window without `openat`/`renameat` |
| Q13 claim-source attribution shadow | `npm run smoke:evidence-gated-answer-claim-attribution` | Verifies Q10-first lazy routing, zero non-sufficient generator/quality/attribution getter access, Q7 v5 flattened grammar, source-bound exact lexical terms, content-free output, and rehashed fixture tamper rejection; it does not assert semantic attribution |
| Q10 local sufficient-answer shadow | `npm run smoke:local-evidence-gated-answer-shadow` | Verifies the tracked loopback qwen2.5:3b sufficient-only generation, frozen answer-quality result, model/runtime provenance, preserved Q9 4/5 failure history, and inactive runtime |
| Q11 local multi-scenario observation | `npm run smoke:local-evidence-gated-answer-robustness` | Verifies four sufficient-only loopback qwen2.5:3b generations, 4/4 frozen synthetic quality results, content-free evidence, pre/post attribution, and false activation, training, user-data, and production claims |
| Q13 local claim-source attribution observation | `npm run smoke:local-evidence-gated-answer-claim-attribution` | Verifies four sufficient-only loopback qwen2.5:3b generations with 4/4 lexical source-term attribution, content-free local observation, and unchanged false semantic-attribution, independent-review, activation, training, user-data, and production claims |
| Council C6 local provider shadow qualification | `npm run smoke:local-council-provider-shadow` | Verifies the tracked loopback qwen2.5:3b sequential observation, three valid but identical openings, three exact rebuttal-target contract failures, dependency-blocked synthesis, content-free evidence, and `keep-stub-only` decision |
...
| Council C11 rebuttal stability and chair reachability shadow | `npm run smoke:local-council-rebuttal-stability-shadow` | Verifies the single unretried loopback observation, research opening `structured-output` failure, six dependency-blocked stages, canonical v5 and fixture bindings, C6–C10 raw file SHA bindings, content-free metrics, exact failure taxonomy, and unchanged `keep-stub-only` authority boundary |
| Council C12 strict prompt candidate qualification | `npm run smoke:local-council-strict-prompt-candidate-qualification` | Verifies the deterministic fake-provider 7/7 v6 contract qualification, first-failure stop, C6–C11 binding, and unchanged `keep-stub-only` authority boundary |
| Council C13 actual compatibility observation | `npm run smoke:local-council-v6-actual-compatibility-observation` | Verifies the existing content-free one-attempt receipt and `actual-incompatible` observation without rerunning the evaluator, provider, or model |
| Council blueprint preview | `npm run smoke:council-blueprint-preview` | Verifies the deterministic v1.1a role catalog and sequential meeting graph through read-only CLI/API surfaces without creating storage or execution authority |
| Council concurrent schedule shadow | `npm run smoke:council-concurrent-schedule-shadow` | Verifies v1.1b four-wave barriers, canonical completion order, sequential parity, fail-closed completion handling, CLI/API equivalence, and empty-root no-write behavior without dispatching work |
...
| Local-first v1 completion closeout | `npm run smoke:local-v1-completion-closeout` | Verifies the implementation SHA, source document hashes, C13 result, six external blockers, false authority fields, and measured repository-local verification results |
