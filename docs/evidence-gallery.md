...
| Local candidate evaluation admission | `evidence/output-artifacts/local-candidate-evaluation-admission.json` | recorded candidate verification, current permission, explicit no-revocation, F1 case·threshold suite, current CPU·memory·disk·runtime envelope, bounded request time window, content-free operator binding, local-evaluation-only authority, and no actual model evaluation, training, external call, or rollout |
| Local candidate evaluation runtime | `evidence/output-artifacts/local-candidate-evaluation-runtime.json` | current authority revalidation, fresh candidate file hashes, evaluator identity, allowlisted local stdio, bounded time and I/O, canonical content-free quality summary, O1a run lineage, fixture-only execution, caller-owned CPU·memory·network isolation, and blocked training and rollout |
