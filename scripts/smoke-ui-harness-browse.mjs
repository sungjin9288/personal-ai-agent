...
try {
  ...
  assert.equal(appJs.includes('data-ui-action="execution-stop"'), true);
  assert.equal(appJs.includes('실행 중단: ${executionSessionLabel}'), true);
  assert.equal(appJs.includes('data-ui-action="execution-rollback-preview"'), true);
  ...
  assert.equal(appJs.includes('aria-pressed="${isCopiedReleaseHandoffSummaryDetail(item.id, detail.key) ? \'true\' : \'false\'}"'), true);
  assert.equal(appJs.includes('aria-pressed="${isCopiedReleaseHandoffSummaryStableLine(item.id, detail.key, lineIndex) ? \'true\' : \'false\'}"'), true);
  assert.equal(appJs.includes('aria-pressed="${structuredSummaryCopied ? \'true\' : \'false\'}"'), true);
