...
try {
  ...
  assert.equal(appJs.includes("container.querySelectorAll('[data-action-open]')"), true);
  assert.equal(appJs.includes('onOpenMission(button.dataset.actionOpen)'), true);
  ...
  assert.equal(appJs.includes('const item = findActionItem(items, button.dataset.actionRerun)'), true);
  assert.equal(appJs.includes('return onRerun(item)'), true);
  ...
  assert.equal(appJs.includes('button.dataset.providerAttentionMode || \'primary\''), true);
  assert.equal(appJs.includes('onProviderAttentionRemediate(item,'), true);
  ...
  assert.equal(appJs.includes('const item = findActionItem(items, button.dataset.specialistFollowUpRemediate)'), true);
  assert.equal(appJs.includes('return onSpecialistFollowUpRemediate(item)'), true);
  ...
  assert.equal(appJs.includes("container.querySelectorAll('[data-action-resolve]')"), true);
  assert.equal(appJs.includes('onReviewerFollowUpResolve(button.dataset.actionResolve)'), true);
  ...
  assert.equal(appJs.includes('export function wireReleaseStatusLifecycleActions({'), true);
  assert.equal(appJs.includes("'run-release-preflight',"), true);
  assert.equal(appJs.includes("'run-release-preflight-all',"), true);
  assert.equal(appJs.includes("'refresh-release-status-live',"), true);
  assert.equal(appJs.includes('preflight: runReleasePreflight'), true);
  ...
  assert.equal(appJs.includes('onDelete: handleDocumentLogDelete'), true);
  assert.equal(appJs.includes('onMigrate: handleLegacyDocumentMigration'), true);
  assert.equal(appJs.includes('export function wireDocumentLogFormActions({ actions, elements, errors })'), true);
  assert.equal(appJs.includes('export function wireMemoryFormActions({ actions, elements, errors })'), true);
  assert.equal(appJs.includes('export function wireMissionFormActions({ actions, elements, errors })'), true);
  assert.equal(appJs.includes('export function wireMissionRunActions({ actions, elements, errors })'), true);
  assert.equal(appJs.includes('export function wireApplicationEvents({ actions, elements, errors, historyTarget })'), true);
  assert.equal(appJs.includes('createDocument: handleDocumentLogCreate'), true);
  assert.equal(appJs.includes('createMissionMemory: handleMemoryCreate'), true);
  assert.equal(appJs.includes('runMission: handleMissionRun'), true);
  assert.equal(appJs.includes("elements.runMissionButton.textContent = '이 미션 실행'"), true);
  assert.equal(appJs.includes('updateFallbackControls: updateRunFallbackControls'), true);
  ...
  assert.equal(appJs.includes("selectStep: (step) => setActiveStep(step, { urlMode: 'push' })"), true);
  assert.equal(appJs.includes("selectDetailTab: (detailTab) => setActiveDetailTab(detailTab, { urlMode: 'push' })"), true);
  assert.equal(appJs.includes('export function wireBrowserHistoryControls({ actions, errors, historyTarget })'), true);
  assert.equal(appJs.includes("listenSafely(historyTarget, 'popstate', actions.restoreHistory, errors.default)"), true);
  assert.equal(appJs.includes('restoreHistory: () => restoreUiStateFromUrl({ syncUrl: false })'), true);
  ...
  assert.equal(appJs.includes('export async function bootstrapApplication({'), true);
  assert.equal(appJs.includes('initializeTheme();\n  wireEvents();\n  renderStaticSurfaces();'), true);
  assert.equal(appJs.includes('await loadData();\n    await restoreState();'), true);
  assert.equal(appJs.includes('wireEvents: attachApplicationEvents'), true);
  assert.equal(appJs.includes('renderStaticSurfaces: renderBootstrapStaticSurfaces'), true);
  assert.equal(appJs.includes('loadData: loadBootstrapData'), true);
  assert.equal(appJs.includes('restoreState: restoreUiStateFromUrl'), true);
  assert.equal(appJs.includes('hydrateBootstrapDataAndRestoreState'), false);
  ...
  assert.equal(appJs.includes('await restoreTargetMissionUrlState(targetMissionId, urlState);'), true);
  assert.equal(appJs.includes('function syncRestoredUiStateToUrl(syncUrl)'), true);
  ...
  assert.equal(appJs.includes('restoreWorkspaceSelectionUrlState(urlState);'), true);
  assert.equal(appJs.includes('await restoreReleaseDetailUrlState(urlState);'), true);
  assert.equal(appJs.includes('createMission: handleMissionCreate'), true);
  assert.equal(appJs.includes("listen(elements.missionForm?.elements?.mode, 'change', actions.renderBlueprint)"), true);
  assert.equal(appJs.includes("listen(elements.missionAttachmentInput, 'change', actions.renderBlueprint)"), true);
  assert.equal(appJs.includes('renderBlueprint: renderAgentBlueprintBuilder'), true);
  assert.equal(appJs.includes('createMissionMemory: handleMemoryCreate'), true);
  assert.equal(appJs.includes('createWorkspaceMemory: handleWorkspaceMemoryCreate'), true);
  assert.equal(appJs.includes("resetMemoryForm('mission')"), true);
  assert.equal(appJs.includes("resetMemoryForm('workspace')"), true);
  assert.equal(appJs.includes('createDocument: handleDocumentLogCreate'), true);
  assert.equal(appJs.includes('searchDocuments: handleHarnessDocumentSearch'), true);
  assert.equal(appJs.includes('filterDocuments: handleHarnessDocumentFilter'), true);
  ...
  assert.equal(appJs.includes('에이전트 진행 흐름이 아직 없습니다'), true);
  ...
  assert.equal(appJs.includes('아직 선택된 미션이 없습니다'), true);
  ...
  assert.equal(appJs.includes('승인 대기 항목이 없습니다'), true);
  ...
  assert.equal(appJs.includes('아직 생성된 미션이 없습니다'), true);
  ...
  assert.equal(appJs.includes('function buildHarnessDocumentsQueryParams(state)'), true);
  assert.equal(appJs.includes('const params = buildHarnessDocumentsQueryParams(state);'), true);
  ...
  assert.equal(appJs.includes('function buildFallbackHarnessDocumentBrowse(harnessSummary = {})'), true);
  assert.equal(appJs.includes('const documentBrowse = state.harnessDocumentResult || buildFallbackHarnessDocumentBrowse(harnessSummary);'), true);
  ...
  assert.equal(appJs.includes('function buildHarnessMemoryQueryParams(state)'), true);
  assert.equal(appJs.includes('const params = buildHarnessMemoryQueryParams(state);'), true);
  ...
  assert.equal(appJs.includes('harness-memory-sort'), true);
  ...
  assert.equal(appJs.includes('selectDocumentFile: handleDocumentLogFilePick'), true);
  ...
  assert.equal(appJs.includes('export function wireHarnessMemoryBrowseActions({'), true);
  assert.equal(appJs.includes('container: elements.harnessMemory'), true);
  ...
  assert.equal(appJs.includes('onDelete: ({ memoryId, scope }) => handleMemoryDelete(scope, memoryId)'), true);
  ...
  assert.equal(appJs.includes('findLearningPromotionItem(items, button.dataset.learningPromotionAuditCopy)'), true);
  assert.equal(appJs.includes('onLearningPromotionAuditCopy(item)'), true);
  ...
  assert.equal(appJs.includes('findLearningPromotionItem(items, button.dataset.learningPromotionResolve)'), true);
  assert.equal(appJs.includes('onLearningPromotionResolve(item, button.dataset.learningPromotionDecision || \'approve\')'), true);
  ...
  assert.equal(appJs.includes('findLearningPromotionItem(items, button.dataset.learningPromotionExpire)'), true);
  assert.equal(appJs.includes('onLearningPromotionExpire(item)'), true);
  ...
  assert.equal(appJs.includes('findLearningPromotionItem(items, button.dataset.learningPromotionRollback)'), true);
  assert.equal(appJs.includes('onLearningPromotionRollback(item)'), true);
  ...
  assert.equal(appJs.includes('actionSummary.innerHTML = unavailableState.summaryHtml'), true);
  assert.equal(appJs.includes('actionList.innerHTML = unavailableState.listHtml'), true);
  assert.equal(appJs.includes('후속 작업 큐가 준비되지 않았습니다'), true);
