...
assert.match(
  contract,
  /identityTenantEvidence \| hosted identity\/session approval, customer IdP onboarding proof, user lifecycle proof, session lifecycle proof, role administration proof, permission propagation proof, immutable audit export proof, break-glass governance proof, support impersonation proof, compliance and retention proof, target identity\/session operations, hosted tenant isolation approval, target tenant isolation operations, RBAC\/session audit, tenant storage boundary, encryption\/key ownership, and cross-tenant denial evidence/,
);
assert.doesNotMatch(
  contract,
