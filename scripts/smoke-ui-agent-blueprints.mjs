...
try {
  ...
  assert.equal(appJs.includes('/api/council/concurrent-retry-terminality-shadow'), true);
  assert.equal(appJs.includes('councilConcurrentRetryTerminalityShadow'), true);
  ...
  assert.equal(appJs.includes('Retry lineage and terminality projection'), true);
  assert.equal(appJs.includes('retryDecision:'), true);
  assert.equal(appJs.includes('actualRetryExecuted: false'), true);
