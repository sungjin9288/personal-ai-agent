...
const sourceReportPath = path.join(outputDir, 'workspace-learning-operator-surface-browser.json');
const sourceScreenshotPath = path.join(outputDir, 'workspace-learning-operator-surface-browser.png');
...
const evidencePath = path.join(evidenceOutputDir, 'workspace-learning-operator-surface.json');
const evidenceScreenshotPath = path.join(
  evidenceScreenshotDir,
  'workspace-learning-operator-surface.png',
);

const report = JSON.parse(fs.readFileSync(sourceReportPath, 'utf8'));
assert.equal(report.mode, 'workspace-learning-operator-surface-browser');
...
const evidence = {
  ...
  mode: 'workspace-learning-operator-surface-evidence',
  results: report.results,
  schemaVersion: 'personal-ai-agent-workspace-learning-operator-surface/v1',
};
...
function readPngDimensions(buffer) {
  ...
  assert.equal(buffer.length >= 24, true, 'workspace learning operator screenshot is truncated');
  assert.equal(buffer.subarray(0, 8).equals(pngSignature), true, 'workspace learning operator screenshot is not PNG');
