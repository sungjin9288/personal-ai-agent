...
assert.doesNotMatch(productionReadySection, /target secret manager alias, injection path/);
assert.doesNotMatch(productionReadySection, /secret injection proof with target secret manager alias/);
assert.doesNotMatch(
  productionReadySection,
...
assert.doesNotMatch(
  releaseReadiness,
  /Hermes remains blocked until target Hermes provider architecture evidence and target-boundary live validation are approved/,
);
...
assert.match(targetSupportArchitecture, /^## Release Blocker Closure Linkage$/m);
assert.match(targetSupportArchitecture, /target-support-architecture-missing/);
assert.match(targetSupportArchitecture, /target-support-operations-missing/);
assert.match(targetSupportArchitecture, /target-support-boundary-missing-or-mismatched/);
assert.match(targetSupportArchitecture, /same-boundary support evidence verification/);
assert.match(targetSupportArchitecture, /regenerated execution-v1 snapshot evidence from the same approved target boundary/);
...
assert.match(targetSloArchitecture, /release artifact hygiene result and regenerated execution snapshot evidence/);
assert.match(targetSloArchitecture, /^## Release Blocker Closure Linkage$/m);
assert.match(targetSloArchitecture, /target-slo-architecture-missing/);
assert.match(targetSloArchitecture, /target-slo-operations-missing/);
assert.match(targetSloArchitecture, /target-observability-slo-boundary-missing-or-mismatched/);
assert.match(targetSloArchitecture, /same-boundary SLO evidence verification/);
assert.match(targetSloArchitecture, /regenerated execution-v1 snapshot evidence from the same approved target boundary/);
...
for (const blocker of [
  /Anthropic live validation remains blocked until target Anthropic provider account evidence for account ownership proof, billing and credit remediation proof, active billing plan proof, available credit balance proof, API key and secret injection proof, ANTHROPIC_MODEL model access proof, provider terms and customer approval proof, quota and spend guard proof, target-boundary live:execution-v1:anthropic proof with mission id, execution session id, provider response status, retry lineage, artifact provenance, and handoff reference, telemetry proof, fallback and stop-condition proof with fallback policy id, stop reason, and recoverable-provider-failure-only stop evidence, provider operations proof, remediation audit proof, release artifact hygiene result, and regenerated execution snapshot is recorded/,
  ...
  /Hermes live validation is blocked until target Hermes provider architecture evidence for endpoint ownership proof, HERMES_PROVIDER_MODEL model pinning proof, target secret injection proof, tool-call parsing proof, session lifecycle proof with mission id, execution session id, provider response id, retry lineage, artifact provenance, and handoff reference, data and transcript policy proof, quota and rate guard proof, telemetry proof, fallback and stop-condition proof with fallback policy id, stop reason, and recoverable-provider-failure-only stop evidence, customer approval proof, provider operations proof, target-boundary live:execution-v1:hermes proof, release artifact hygiene result, and regenerated execution snapshot is recorded/,
