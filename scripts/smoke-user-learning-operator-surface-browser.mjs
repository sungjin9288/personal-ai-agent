...
const tempRoot = fs.mkdtempSync(
  path.join(os.tmpdir(), 'personal-ai-agent-user-learning-operator-surface-browser-'),
);
const outputDir = path.join(repoDir, 'output', 'playwright');
const reportPath = path.join(outputDir, 'user-learning-operator-surface-browser.json');
const screenshotPath = path.join(outputDir, 'user-learning-operator-surface-browser.png');
const sessionId = `user-learning-operator-surface-${Date.now().toString(36)}`;
...
const workspace = runCli({
  rootDir: tempRoot,
  args: ['workspace', 'add', workspacePath, '--name', 'user-learning-operator-surface-browser'],
});
const mission = runCli({
  ...
  args: [
    ...
    'decision-memo',
    '--title',
    'User learning operator surface browser',
    '--objective',
    'Verify the permission-bound local user learning override controls in a real browser.',
  ],
});
...
const authorization = runCli({
  ...
  args: [
    ...
    'user',
    '--note',
    'Authorize the reviewed browser fixture for local user reuse.',
  ],
});
...
const promotion = runCli({
  ...
  args: [
    ...
    'user',
    '--note',
    'Promote the reviewed browser fixture as a local user decision.',
  ],
});
...
try {
  ...
  const browserResult = runPwJson([
    ...
    `async (page) => {
      ...
      const setSelector = '[data-user-learning-selection-override-set="' + candidateId + '"]';
      const clearSelector = '[data-user-learning-selection-override-clear="' + candidateId + '"]';
      const errors = [];
      ...
      await page.evaluate(({ expiresAt }) => {
        const answers = [expiresAt, 'Pin the reviewed user decision through the real browser control.'];
        window.prompt = () => answers.shift() || '';
      }, { expiresAt });
      await page.locator(setSelector).click();
      await page.waitForFunction(
        () => document.querySelector('#action-list')?.textContent?.includes('user selection override active'),
        null,
        { timeout: 15000 },
      );
      ...
      await page.waitForFunction(
        () => document.querySelector('#action-list')?.textContent?.includes('user selection override expired'),
        null,
        { timeout: 15000 },
      );
      ...
      await page.evaluate(() => {
        const answers = ['Return to latest user revision through the real browser control.'];
        window.prompt = () => answers.shift() || '';
      });
      await page.locator(clearSelector).click();
      await page.waitForFunction(
        () => document.querySelector('#action-list')?.textContent?.includes('user selection override cleared'),
        null,
        { timeout: 15000 },
      );
      ...
      return {
        activeVisible: activeText.includes('user selection override active'),
        clearButtonCount: await page.locator(clearSelector).count(),
        clearedVisible: clearedText.includes('user selection override cleared'),
        expiredVisible: expiredText.includes('user selection override expired'),
  ...
  const report = {
    ...
    mode: 'user-learning-operator-surface-browser',
    ok: true,
