...
export const WORKSPACE_LEARNING_CONFLICT_REVOCATION_SCHEMA_VERSION =
  'personal-ai-agent-workspace-learning-conflict-revocation/v1';
...
function requireHash(value, fieldName) {
  ...
  if (!/^[a-f0-9]{64}$/.test(normalized)) {
    throw new Error(`Workspace learning conflict ${fieldName} is invalid.`);
  }
  return normalized;
}
...
function normalizeBinding(binding = {}) {
  ...
  if (!normalized.caseId) {
    throw new Error('Workspace learning conflict caseId is required.');
  }
...
function normalizeMission(mission = {}, label) {
  ...
  if (!normalized.id || !normalized.workspaceId) {
    throw new Error(`Workspace learning conflict ${label} mission is incomplete.`);
  }
  return normalized;
}
...
function normalizeTopology(topology = {}) {
  ...
  if (!normalized.foreignWorkspaceId || !normalized.sourceWorkspaceId) {
    throw new Error('Workspace learning conflict topology is incomplete.');
  }
  return normalized;
}
...
function normalizePromotion(promotion = {}, label) {
  ...
  if (
    ...
    throw new Error(`Workspace learning conflict ${label} promotion is incomplete.`);
  }
  return normalized;
}
...
function normalizeLifecycle(lifecycle = {}, label) {
  ...
  if (
    ...
    throw new Error(`Workspace learning conflict ${label} lifecycle is incomplete.`);
  }
  return normalized;
}
...
function normalizeSelection(selection, label) {
  ...
  const normalized = {
    ...
    workspaceId: normalizeText(selection.workspaceId),
  };
  if (
    !normalized.policyId ||
    !normalized.reason ||
    ...
    throw new Error(`Workspace learning conflict ${label} selection is incomplete.`);
  }
  return normalized;
}
...
function normalizeRun(run = {}, label) {
  ...
  if (!normalized.providerId || !normalized.reviewerVerdict || !normalized.sessionId || !normalized.status) {
    throw new Error(`Workspace learning conflict ${label} run is incomplete.`);
  }
  return normalized;
}
...
function normalizeQuality(result = {}, label) {
  ...
  if (!normalized.id || !['passed', 'failed'].includes(normalized.status)) {
    throw new Error(`Workspace learning conflict ${label} quality is invalid.`);
  }
...
function selectionMatches(run, promotion, candidateCount) {
  return (
    ...
    run.selection?.policyId === 'workspace-decision-latest-revision-v1' &&
...
function promotionMatchesWorkspace({
  ...
  return (
    ...
    promotion.scopeAuthorizationToScopeId === workspaceId &&
...
function buildContent(input = {}) {
  ...
  const phases = {
    ...
    foreignConflict: normalizeRun(input.phases?.foreignConflict, 'foreignConflict'),
  ...
  const quality = {
    ...
    foreignConflict: normalizeQuality(input.quality?.foreignConflict, 'foreignConflict'),
  ...
  const promotionsVerified =
    promotionMatchesWorkspace({
      ...
      workspaceId: topology.sourceWorkspaceId,
    }) &&
    promotionMatchesWorkspace({
  ...
  const foreignWorkspaceIsolated =
    phases.foreignConflict.selection === null &&
    hasNoExposure(phases.foreignConflict) &&
    phases.foreignConflict.artifacts.retrievalHash === null &&
    quality.foreignConflict.status === 'passed';
  ...
  return {
    actualWorkspaceLearningConflictRevocationValidated: validated,
    costFree: true,
    ...
    qualityBoundary: {
      ...
      controlledConflictResolutionValidated: validated,
      controlledRevocationFallbackValidated: validated,
    ...
    results,
    schemaVersion: WORKSPACE_LEARNING_CONFLICT_REVOCATION_SCHEMA_VERSION,
    sourceRuns,
    status: validated
      ? 'workspace-learning-conflict-revocation-passed-local-only'
      : 'failed-keep-single-workspace-decision-only',
    topology,
  };
}

export function buildWorkspaceLearningConflictRevocationEvidence(input = {}) {
  const content = buildContent(input);
  const evidenceHash = hashRecord(content);
  return {
    ...content,
    evidenceHash,
    id: `workspace-learning-conflict-revocation-${evidenceHash}`,
  };
}

export function assertWorkspaceLearningConflictRevocationEvidence(evidence) {
  const { evidenceHash, id, ...content } = evidence || {};
  const expectedHash = hashRecord(content);
  const errors = [];
  if (
    evidenceHash !== expectedHash ||
    id !== `workspace-learning-conflict-revocation-${expectedHash}`
  ) {
  ...
  if (errors.length) {
    throw new Error(
      `Workspace learning conflict evidence failed: ${[...new Set(errors)].join(', ')}.`,
    );
  }
}
