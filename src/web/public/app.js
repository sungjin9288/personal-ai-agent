...
function getReleaseSharedProviderOperationsScopeAudit({
  totalActions = getReleaseCurrentOpenBlockerActions(),
  category = state.releaseBlockerCategoryFilter,
  includeShared = true,
  owner = state.releaseBlockerOwnerFilter,
...
function buildReleaseSharedProviderOperationsScopeAuditLines(options = {}) {
...
function renderMissionActions() {
  ...
  elements.actionSummary.innerHTML = `
    ...
    <div class="action-row action-filter-row">
      ...
      <select data-action-inbox-fallback-stop-filter="true" aria-label="fallback stop reason filter" ${hasFallbackStopReasonOptions ? '' : 'disabled'}>
