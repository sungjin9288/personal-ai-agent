...
const releaseEvidenceTriageCopyButtons = {
  renderReleaseBlockerClosureChecklistCopyButton,
  renderReleaseBlockerClosureMatrixCopyButton,
  renderReleaseBlockerCommandsCopyButton,
  renderReleaseBlockerEvidenceCopyButton,
...
export function isReleaseBlockerActionVisibleForCopyScope(
  blockerAction = null,
  {
    category = state.releaseBlockerCategoryFilter,
    includeShared = state.releaseBlockerIncludeSharedProviderOperations,
...
export async function copyUiLink(url, {
...
export function buildProviderFallbackEventAuditPackageText(payload = state.providerEvents) {
  ...
  elements.releaseStatus.innerHTML = `
    <div class="release-status-shell">
      ${renderReleaseStatusOverview({
        ...
        releaseSnapshotPreflight,
        renderCommandCopyButton: renderReleaseCommandCopyButton,
        snapshotConfirmArmed,
        snapshotEligibility,
      })}

      ${renderReleaseVerificationSurfaces({ release })}
      ${renderReleaseRuntimeJobs({ release })}

      <div class="detail-grid detail-grid-two release-detail-grid">
        ${renderReleaseCloseoutChecklist({ release })}
        ...
        <section class="surface">
          ...
          <div class="release-list">
            ...
            ${renderReleaseCurrentOpenBlockers({
              ...
              copyButtons: releaseEvidenceTriageCopyButtons,
              view: evidenceTriageView,
            })}
            ${renderReleaseProductionBlockerDetails({
              gaps,
...
async function handleWorkspaceLearningSelectionOverrideSet(item) {
  const defaultExpiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString();
  const expiresAt = window.prompt('선택 고정 만료 시각을 ISO 형식으로 입력하세요.', defaultExpiresAt);
  if (!expiresAt) {
    return;
  }
  ...
  const note = window.prompt(
    '선택 고정 사유를 입력하세요.',
    'UI에서 검증된 workspace decision을 제한된 기간 동안 고정',
  );
  if (!note) {
    return;
  }
  ...
  const candidateId = getLearningPromotionCandidateId(item);
  await api(
    `/api/actions/learning-promotions/${encodeURIComponent(candidateId)}/workspace-selection-override`,
    {
...
async function handleWorkspaceLearningSelectionOverrideClear(item) {
  const note = window.prompt(
    '선택 고정 해제 사유를 입력하세요.',
    'UI에서 latest revision 자동 선택으로 복귀',
  );
  if (!note) {
    return;
  }
  ...
  const candidateId = getLearningPromotionCandidateId(item);
  await api(
    `/api/actions/learning-promotions/${encodeURIComponent(candidateId)}/workspace-selection-override/clear`,
    {
      body: JSON.stringify({ note }),
      method: 'POST',
    },
