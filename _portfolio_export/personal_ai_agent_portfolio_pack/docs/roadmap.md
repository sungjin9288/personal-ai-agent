...
- v1.1f retry terminality operator surface: existing v1.1e output is exposed through additive CLI, GET, and Council preview read-only rendering. CLI/GET parity, malformed·duplicate·stale fail-closed inputs, three-seat terminality matrix, four-through-seven-seat denial, and request-audit-only HTTP behavior are verified. No retry executor, scheduler, POST/button, store schema, provider route, external/model/Ollama/C13/worker/network call, or actual user data is introduced; `keep-retry-disabled`, `keep-dispatch-disabled`, `keep-stub-only`, and `productionReadyClaim: false` remain fixed.
- v1.1f deterministic CI closeout: the unchanged Provider workflow and required-job identity run six focused Council unit files plus one deterministic stable-snapshot regression, the six Council deterministic smokes, and the UI blueprint smoke directly. Contributor onboarding checks the exact seven-file command and smoke order and performs eight in-memory removal negative checks that must fail closed; it does not configure branch protection or infer required-check status beyond live GitHub evidence. No runtime, provider, model, Ollama, C13, retry, dispatch, worker, network, schema, permission, approval, or audit behavior changes.
