...
- 현재 구현 완료: Node.js ESM CLI/web runtime, local JSON store, workspace/mission/session/artifact/approval model, provider registry/adapters, D4 service refactoring, local RAG fixture/shadow contracts, Council C1–C13 contracts and observations, v1.1a/v1.1b read-only schedule projections, v1.1c deterministic structural envelope shadow, smoke scripts, GitHub Actions provider smoke
