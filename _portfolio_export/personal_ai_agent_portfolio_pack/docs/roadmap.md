...
- v1.1f deterministic CI closeout: the unchanged Provider workflow and required-job identity run the six focused Council unit files, the six Council deterministic smokes, and the UI blueprint smoke directly. Contributor onboarding checks exact command order and performs eight in-memory removal negative checks that must fail closed; it does not configure branch protection or infer required-check status beyond live GitHub evidence. No runtime, provider, model, Ollama, C13, retry, dispatch, worker, network, schema, permission, approval, or audit behavior changes.
