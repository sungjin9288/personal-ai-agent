...
| Local training runtime contract | `evidence/output-artifacts/local-training-runtime-contract.json` | exact F1 and expiring approval binding, shell-free local stdio, environment and resource bounds, content-free candidate metadata, ten failure guards, no store mutation, and no actual model training or rollout authority |
...
| Local training post-acquisition readiness | `evidence/output-artifacts/local-training-post-acquisition-readiness.json` | fixture-only provenance, egress closure, offline resource canary, and post-install permission evidence binding with owner, timestamp, resource, and hash failure guards; all actual gates and training authority remain false |
