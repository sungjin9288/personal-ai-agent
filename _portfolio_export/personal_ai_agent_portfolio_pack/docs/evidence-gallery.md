...
| Local candidate evaluation runtime | `evidence/output-artifacts/local-candidate-evaluation-runtime.json` | current authority revalidation, manifest-listed candidate snapshot, exact suite bytes, pre/post input hashes, cleanup-before-evidence, evaluator identity, allowlisted local stdio, bounded time and I/O, canonical content-free quality summary, O1a run lineage, fixture-only execution, caller-owned executable provenance and OS resource·network isolation, and blocked training and rollout |
