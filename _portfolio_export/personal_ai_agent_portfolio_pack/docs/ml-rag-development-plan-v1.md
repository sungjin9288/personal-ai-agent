...
- currentCostFreeMilestone: mlx-lm-lora-training-adapter-contract
...
Tracked replay는 `child_process`를 사용하지 않는 내부 deterministic fixture writer로 data·argv·candidate pipeline만 검증한다. Workspace cleanup 실패 시 published candidate를 제거하고 성공 observation을 남기지 않지만, cleanup과 rollback이 함께 실패한 orphan을 추적·회수하는 durable recovery ledger와 explicit cleanup은 아직 없다. 실제 mode는 이 recovery gate와 OS-enforced no-egress, CPU·memory limit, detached process-group lifecycle, permission revocation monitoring, complete Python runtime closure provenance가 구현되기 전까지 constructor에서 거부한다. 따라서 `actualMlxLmLoraAdapterContractValidated: true`이지만 `actualMlxProcessSpawned: false`, `actualModelTrainingExecuted: false`, `trainingAuthorized: false`, `productionReadyClaim: false`다.
...
| F2c.20 MLX-LM LoRA adapter contract | 완료 · fixture 증적 | F2a admission 뒤 pinned local paths, exact F1 train·valid bytes, fixed offline argv·env, complete acquisition inventory, candidate manifest와 atomic publish를 fixture로 재생 | actual process mode는 OS egress·resource·process lifecycle·revocation monitoring·runtime closure provenance가 마련될 때까지 차단; 설치·download·training·rollout 미실행 |
