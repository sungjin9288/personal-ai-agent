...
- currentCostFreeMilestone: local-training-failure-recovery
...
Tracked replay는 `child_process`를 사용하지 않는 내부 deterministic fixture writer로 data·argv·candidate pipeline만 검증한다. F2c.21이 workspace cleanup과 candidate rollback의 durable recovery를 맡으므로 이 단계의 남은 actual-mode gate는 OS-enforced no-egress, CPU·memory limit, detached process-group lifecycle, permission revocation monitoring, complete Python runtime closure provenance다. 따라서 `actualMlxLmLoraAdapterContractValidated: true`이지만 `actualMlxProcessSpawned: false`, `actualModelTrainingExecuted: false`, `trainingAuthorized: false`, `productionReadyClaim: false`다.
