...
| F2b 실제 local model training | 승인 작업 | 승인된 base model·license·resource·egress 경계와 operator permission surface가 준비된 뒤 같은 protocol로 실행 | actual model artifact, resource evidence, rollback owner와 candidate evaluation evidence 기록 |
| F2c 외부 fine-tuning 실행 | 외부 작업 | 승인된 provider·budget·model이 있을 때 별도 adapter로 제출 | 명시 승인, 비용 한도, model id, 결과, rollback 기록 |
...
node --test test/local-training-runtime.test.mjs
