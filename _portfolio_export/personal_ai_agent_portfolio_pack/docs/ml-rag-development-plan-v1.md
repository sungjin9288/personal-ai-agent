...
- currentCostFreeMilestone: local-training-process-supervisor
...
Tracked replay는 `child_process`를 사용하지 않는 내부 deterministic fixture writer로 data·argv·candidate pipeline만 검증한다. F2c.21이 workspace cleanup과 candidate rollback의 durable recovery를 맡고 F2c.22가 static Python runtime closure를 결속한다. F2c.23은 별도 fixture process supervisor의 lifecycle과 권한 철회 감시를 검증하지만 MLX invocation에는 아직 연결하지 않는다. 남은 actual-mode gate는 supervisor integration, OS-enforced no-egress, CPU·memory limit, native dependency closure와 verify-to-exec binding이다. 따라서 `actualMlxLmLoraAdapterContractValidated: true`이지만 `actualMlxProcessSpawned: false`, `actualModelTrainingExecuted: false`, `trainingAuthorized: false`, `productionReadyClaim: false`다.
