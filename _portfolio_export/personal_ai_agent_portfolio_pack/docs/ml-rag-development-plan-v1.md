...
이 smoke의 command는 deterministic concept fixture일 뿐 실제 ML embedding model이 아니다. Adapter가 child process의 network를 직접 차단하지 않으므로 실제 command의 binary provenance, model license·version, network isolation, resource limit과 같은 retrieval suite 품질 검증은 operator가 별도 증적으로 남겨야 한다. 따라서 `actualLocalEmbeddingModelValidated: false`, `productionReadyClaim: false`를 유지한다.
