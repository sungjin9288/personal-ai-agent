...
- scope: provider account proof, target provider architecture proof, closed bounded pilot feedback, hosted production proof
...
| Sanitized single-participant deterministic pilot feedback | One consenting engineering participant completed one approved non-sensitive deterministic-only session; [record](../config/pilot-feedback-v1.json) and [readable evidence](pilot-feedback-v1.md) retain predefined metrics, content-free hashes, and limitations | `npm run smoke:pilot-feedback`; 8/8 demo steps, 4/4 stub role runs, 4/4 binary feedback answers, zero external provider calls, zero workspace mutations | The bounded pilot feedback gap is closed for this one session only; no external-provider validation, customer impact, productivity, cost saving, SLA, generalizability, or production-readiness claim follows |
...
- One sanitized n=1 deterministic-only pilot feedback record exists; raw local artifacts and participant identity are not published.
...
- One consenting engineering participant's bounded deterministic-only feedback is verified with exact sanitized evidence and a fail-closed smoke.
- Anthropic, Hermes, target local provider, and hosted deployment claims remain blocked until evidence is supplied.
...
This register is current only when `npm run smoke:external-evidence-blockers`, `npm run smoke:pilot-feedback`, `npm run smoke:provider-readiness-matrix`, `npm run smoke:smoke-validation-summary`, and `npm run smoke:recorded-walkthrough` pass, the closed walkthrough and pilot records retain their exact identities and claim boundaries, and every remaining row keeps blocked external evidence out of the safe claim set until the named closing evidence exists.
