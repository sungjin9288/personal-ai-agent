...
| Scenario 2 engineering approval rehearsal | 검증 완료 · deterministic fixture only | `evidence/output-artifacts/engineering-approval-workflow-rehearsal.json`, `docs/engineering-approval-workflow-rehearsal-v1.md` | isolated temp workspace에서 create/run/show/timeline, approval/action inbox, reviewer pass, fixture approval, cleanup 검증 | participant 0, human approval/feedback false, external call/cost 0, productionReadyClaim false |
...
| Operator surface browser screenshots | 검증 완료 | `evidence/screenshots/operator-surface-mission-run.png`, `evidence/screenshots/operator-surface-provider-readiness.png`, `evidence/screenshots/operator-surface-action-inbox.png` | Playwright capture report records active UI tab/step, screenshot size, and SHA-256 | local pilot browser evidence, not a hosted interactive demo or production service |
