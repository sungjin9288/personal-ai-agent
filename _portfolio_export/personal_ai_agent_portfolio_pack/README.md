...
- F2c.12 adds the bounded local candidate evaluation runtime (`npm run smoke:local-candidate-evaluation-runtime`). It rechecks current authority, independently rehashes the admitted candidate files, runs an evaluator through allowlisted local stdio with shell disabled and bounded time and I/O, rejects raw answer output, and binds the canonical quality summary to an O1a evidence record. The tracked replay is fixture-simulated; CPU, memory, and network isolation remain caller-owned, the suite descriptor does not independently pin raw suite bytes, a small verify-to-open race remains, and model rollout stays blocked.
