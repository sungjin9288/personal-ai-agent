...
- The private collection item admission protocol revalidates the exact F1.1–F1.8 chain and binds sixteen hashes, including the exact empty workspace and one content-free envelope (`npm run smoke:fine-tuning-private-collection-item-admission`). It accepts only opaque lineage, usage, consent, redaction, and retention references; duplicate references, lane capacity, expiry, retention, history drift, and any workspace mutation fail closed. It creates neither content nor a collected record or case, and identity, evidence, consent, redaction, retention, candidate review, training, provider submission, rollout, and production claims remain unverified or unauthorized.
