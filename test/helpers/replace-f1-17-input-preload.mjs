...
const target = process.env.FINE_TUNING_F1_17_REPLACE_INPUT;
const mode = process.env.FINE_TUNING_F1_17_REPLACE_MODE;
const originalMkdirSync = fs.mkdirSync;
const originalRenameSync = fs.renameSync;
let replaced = false;

fs.mkdirSync = function replaceInputAfterPendingCreation(directory, ...args) {
  ...
  if (
    ...
    if (mode === 'bytes') {
      ...
      const replacement = path.join(
        path.dirname(target),
        '.f1-17-same-byte-replacement.json',
      );
