...
function selection(selected, other = null) {
  ...
  return {
    ...
    candidates,
    policyId: 'user-decision-latest-revision-v1',
    productionReadyClaim: false,
    reason: 'latest effective timestamp, then descending memory id',
    schemaVersion: 'personal-ai-agent-user-learning-selection/v1',
    scope: 'user',
    scopeId: 'user',
...
function buildInput() {
  ...
  return {
    ...
    phases: {
      ...
      crossWorkspaceConflict: run({
        artifactPhase: 'cross-newer',
        label: 'cross-workspace-conflict',
        newerApplied: true,
        other: older,
        selected: newer,
      }),
    ...
    quality: {
      ...
      crossWorkspaceConflict: quality('newer-quality', 'passed'),
...
test('user learning conflict selects newer, falls back after revocation, and restores baseline', () => {
  const evidence = buildUserLearningConflictRevocationEvidence(buildInput());
  ...
  assert.equal(evidence.results.crossWorkspaceConflictApplied, true);
...
test('user learning conflict fails closed on stale selection or rollback drift', () => {
  ...
  const staleEvidence = buildUserLearningConflictRevocationEvidence(stale);
  assert.equal(staleEvidence.actualUserLearningConflictRevocationValidated, false);
  ...
  const driftedEvidence = buildUserLearningConflictRevocationEvidence(drifted);
  assert.equal(driftedEvidence.actualUserLearningConflictRevocationValidated, false);
  assert.equal(driftedEvidence.results.fullRollbackRestoresBaseline, false);
});

test('user learning conflict rejects malformed and tampered evidence', () => {
  const malformed = buildInput();
  malformed.promotions.newer.memoryContentHash = 'invalid';
  assert.throws(
    () => buildUserLearningConflictRevocationEvidence(malformed),
    /memoryContentHash/,
  );
  ...
  assert.throws(
    () => buildUserLearningConflictRevocationEvidence(invalidRate),
    /retrievalHitRate/,
  );

  const tampered = buildUserLearningConflictRevocationEvidence(buildInput());
