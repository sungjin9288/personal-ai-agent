...
test('v1 through v5 freeze every opening, rebuttal, and chair prompt byte and hash', () => {
  for (const [profile, frozen] of Object.entries(expected)) {
    ...
    assert.deepEqual(actual, frozen, profile);
  }
});

function inputs() {
  const openings = seats.map((councilSeatId) => ({
    councilFrame: { evidenceCatalog: [{ id: 'artifact:bounded-plan' }, { id: 'artifact:verification-record' }] },
    councilPhase: 'opening-position', councilSeatId, role: 'specialist',
  }));
  const rebuttals = seats.map((councilSeatId) => ({
    councilBrief: { claims: seats.map((seatId) => ({ id: `${seatId}:claim-1`, seatId })), evidenceRefs: ['artifact:bounded-plan', 'artifact:verification-record'] },
    councilPhase: 'rebuttal', councilSeatId, role: 'specialist',
  }));
  return [...openings, ...rebuttals, {
    ...
    councilSynthesisInput: {
      brief: { claims: seats.map((seatId) => ({ id: `${seatId}:claim-1`, seatId })), evidenceRefs: ['artifact:bounded-plan', 'artifact:verification-record'] },
      rebuttals: seats.map((seatId) => ({ councilStatement: { claims: [{ id: `${seatId}:claim-2` }] } })),
    },
  }];
}
