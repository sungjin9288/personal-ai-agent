...
import { createUserLearningSelectionService } from '../src/core/user-learning-selection-service.mjs';
...
function createFixture({ candidate = buildEligibleCandidate(), tenantId = '' } = {}) {
  const calls = [];
  let storedCandidate = structuredClone(candidate);
  const service = createUserLearningSelectionService({
...
test('user learning override records permission-bound set and clear history in order', () => {
  const fixture = createFixture();

  const setResult = fixture.service.setUserLearningSelectionOverride('candidate-older', {
    expiresAt: '2026-07-17T02:00:00.000Z',
    note: 'Keep the reviewed broad user decision during the incident window.',
  });
  assert.equal(setResult.selectionOverride.status, 'active');
  assert.equal(setResult.selectionOverride.setBy, 'local-operator');
  assert.equal(setResult.selectionOverride.scope, 'user');
  assert.equal(setResult.selectionOverride.scopeId, 'user');
  ...
  const clearResult = fixture.service.clearUserLearningSelectionOverride('candidate-older', {
    note: 'Return to the latest reviewed user decision.',
  });
  ...
  assert.deepEqual(
    fixture.getCandidate().userLearningSelectionOverrideHistory.map((entry) => entry.action),
    ['set', 'clear'],
  );
  assert.deepEqual(fixture.calls, ['store:candidate-older', 'artifact:candidate-older']);
});

test('user learning override read model is content-free and records its local source workspace', () => {
  const fixture = createFixture();

  assert.deepEqual(
    fixture.service.getUserLearningSelectionOverrideReadModel('candidate-older'),
    {
  ...
  fixture.service.setUserLearningSelectionOverride('candidate-older', {
    expiresAt: '2026-07-17T02:00:00.000Z',
    note: 'Keep the reviewed decision for one hour.',
  });
  const readModel = fixture.service.getUserLearningSelectionOverrideReadModel('candidate-older');
...
test('user learning override hides ineligible promotions', () => {
  const candidate = buildEligibleCandidate();
  candidate.promotionVerification.status = 'failed';
  const fixture = createFixture({ candidate });

  assert.equal(fixture.service.getUserLearningSelectionOverrideReadModel('candidate-older'), null);
});

test('user learning override rejects tenant-bound source workspaces before mutation', () => {
  const fixture = createFixture({ tenantId: 'tenant-a' });

  assert.throws(
    () => fixture.service.setUserLearningSelectionOverride('candidate-older', {
      expiresAt: '2026-07-17T02:00:00.000Z',
      note: 'Do not cross the tenant boundary.',
    }),
...
test('user learning override validates permission evidence before mutation', () => {
  const candidate = buildEligibleCandidate();
  candidate.promotionScopeAuthorization.status = 'pending';
  const fixture = createFixture({ candidate });
...
test('user learning override rejects empty notes and non-future expiration before mutation', () => {
  const fixture = createFixture();
...
test('user learning override clear requires an active override and explicit note', () => {
  const fixture = createFixture();
  assert.throws(
    () => fixture.service.clearUserLearningSelectionOverride('candidate-older', {
...
test('user learning override rejects secret and raw customer notes before mutation', () => {
  const fixture = createFixture();
