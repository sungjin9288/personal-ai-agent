...
- release 탭의 `검토용 artifact 바로가기`는 위 browser report/screenshot과 release-doc `index/manifest/digest` artifact 경로를 같은 surface에서 노출하고, smoke는 최소 `index.md`/`index.txt`/`index.json` handoff card의 존재와 non-empty path까지 같이 잠금
