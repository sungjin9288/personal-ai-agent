...
- `provider activity` summary now also includes execution `dailyBuckets` plus `latestBucketDelta`, so recent provider spend trend can be read as a small per-day aggregate without rebuilding timeline data client-side.
