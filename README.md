...
- release 탭의 `검토용 artifact 바로가기`는 위 browser report/screenshot과 release-doc `index/manifest/digest` artifact 경로를 같은 surface에서 노출하고, 존재하는 artifact는 `열기` 링크로 바로 새 탭에서 확인 가능하며 `markdown/text/json` 계열은 `미리보기`로 같은 탭 안에서 inline preview도 바로 확인 가능함. 선택한 preview는 `rartifact` query state로도 동기화돼 reload 뒤에도 같은 panel을 복원함
- browser smoke는 temp workspace에 deterministic handoff fixture를 먼저 심은 뒤 최소 `index.md`/`index.txt`/`index.json` card가 실제 `ready` badge, non-empty path, `/api/execution-v1/handoff-artifacts/:id` direct-open href, `200 + expected content-type` 응답, release 탭 안의 inline preview 본문, 그리고 `rartifact=index-markdown` reload restore까지 모두 만족하는지 같이 잠금
