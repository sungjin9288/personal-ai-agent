...
- release blocker handoff can be inspected without the web console using `node src/cli.mjs overview release-blockers --provider hermes`, which returns provider-specific stop reasons plus the shared provider-operations closure requirements unless `--without-shared` is set; verify this surface with `npm run smoke:release-blocker-handoff`
