...
- The private answer-quality case materialization protocol accepts only the canonical approved F1.17 resolution, live F1.16 candidate, and owner-only enrichment input (`npm run smoke:fine-tuning-private-answer-quality-case`). It rebuilds retrieval and runs the fixed local Q1 evaluator before publishing one content-free logical case. The record stores only hashes, counts, and metrics; payload lifecycle, actual training, provider use, submission, deployment, and production claims remain absent.
