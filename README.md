...
provider-supplied execution manifest에도 같은 정리가 적용됩니다. `TBD_AFTER_INSPECTION`, `e.g. ...`, `or equivalent` 같은 placeholder command는 실행 대상에서 제거하고, 그 결과 verification step이 비면 기본 `node --check src/cli.mjs` smoke로 대체합니다. 즉, live provider가 “나중에 채울 명령”을 남겨도 foreground execution session이 그 문자열을 그대로 shell에 던지지 않습니다.
