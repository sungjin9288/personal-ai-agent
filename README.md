...
Learning candidates now have a read-only operator audit packet: `overview learning-candidates` summarizes promotion status, record type, proposed target, scope, provider fallback lesson evidence, retention/expiration policy, rollback eligibility, gateway event bindings, and no-raw-secrets/no-raw-customer-payload counters while keeping autonomous promotion disabled.
