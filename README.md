...
- release blocker handoff can be inspected without the web console using `node src/cli.mjs overview release-blockers --provider hermes`, or from the operator API with `/api/execution-v1/release-blockers?provider=hermes`; both return provider-specific stop reasons plus the shared provider-operations closure requirements unless `--without-shared`, `withoutShared=true`, or `includeShared=false` is set; verify this surface with `npm run smoke:release-blocker-handoff`
