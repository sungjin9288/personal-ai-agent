...
이번 단계부터 item-level `workspaceHealthDrift`도 `workspaceProfileCounts`와 `workspaceStatusCounts`를 같이 반환하므로 per-profile workspace health pressure 분포를 root `workspaceHealthDrift`와 같은 field shape로 바로 읽을 수 있습니다.
