...
- `provider events` merges persisted probe events, provider execution events, and provider attention acknowledgement, recovery, resolution, reminder events into one chronological stream, with `--family <probe|execution|attention>` plus probe- and execution-specific filters.
