...
- Council v1.1d is a deterministic core-only retry-lineage projection. It binds canonical v1.1b completion and v1.1c envelope content, then records at most one hypothetical attempt 2 lineage for the first failed or timed-out stage. It does not authorize or execute retry, dispatch, provider/model/Ollama/C13/network work, or filesystem/store writes.
...
- The private collection plan protocol binds one current F1.4 attestation to the exact F1.1–F1.4 hashes, F1.3 targets, request expiry, and seven pending preparation steps (`npm run smoke:fine-tuning-private-collection-plan`). It writes only an owner-only private plan, creates no workspace or data, and requires a separate collection execution approval before any reviewed-example or answer-quality collection. Candidate review, training, provider submission, rollout, and production claims remain unauthorized.
- The private collection execution request protocol revalidates the exact F1.1–F1.5 chain and binds the current targets to seven ordered actions and five pending owner reviews (`npm run smoke:fine-tuning-private-collection-execution-request`). It writes one owner-only request per private plan but records no actual request in Git and creates no workspace or data. Owner identity, evidence verification, collection execution, candidate review, training, provider submission, rollout, and production claims remain unauthorized.
- The private collection execution resolution protocol revalidates the exact F1.1–F1.6 chain and preserves the inherited expiry, targets, seven ordered actions, five ordered reviews, and hash-only reasons (`npm run smoke:fine-tuning-private-collection-execution-resolution`). Unanimous private attestation enables only bounded private workspace preparation and reviewed-example and answer-quality collection authority; rejection enables none. No actual operator request, decision, or resolution is Git-tracked; the CLI writes only an owner-only content-free protocol record and creates no workspace or data. Owner identity, evidence content, candidate review, training, provider submission, rollout, and production claims remain unverified or unauthorized.
- The private collection workspace protocol revalidates the exact F1.1–F1.7 chain and binds fourteen hashes, the inherited expiry, targets, and seven ordered actions to two empty owner-only lanes (`npm run smoke:fine-tuning-private-collection-workspace`). It creates no collected record and does not start collection. No actual approval, resolution, workspace, or data is Git-tracked; owner identity, evidence content, candidate review, training, provider submission, rollout, and production claims remain unverified or unauthorized.
- The private collection item admission protocol revalidates the exact F1.1–F1.8 chain and binds sixteen hashes, including the fixed workspace and one content-free envelope (`npm run smoke:fine-tuning-private-collection-item-admission`). It accepts only opaque lineage, usage, consent, redaction, and retention references; duplicate references, lane capacity, expiry, retention, history drift, partial workspace content, and unrecognized workspace entries fail closed. It creates neither content nor a collected record or case, and identity, evidence, consent, redaction, retention, candidate review, training, provider submission, rollout, and production claims remain unverified or unauthorized.
- The private collection item write protocol revalidates the exact F1.1–F1.9 chain before storing one sanitized item per admission in its immutable workspace lane (`npm run smoke:fine-tuning-private-collection-item`). It permits only the explicit origin-consent matrix, bounds and normalizes text, rejects raw JSON, secrets, customer identifiers, email, phone, and display controls, and keeps `workspace.json` byte-identical. It uses a private shared lock, an atomic owner-only write, exact admission/workspace path and inode checks, and a content-free withdrawal/deletion tombstone read gate bound to the admission, evidence hash, and retention-deletion role. The deterministic scan is not independent deidentification proof; no approved record, case, candidate review, training, provider submission, deletion execution, rollout, or production claim is made.
- The private collection item lifecycle accepts one owner-attested withdrawal or retention-delete decision only for the exact stored item, admission, workspace, and retention reference (`npm run smoke:fine-tuning-private-collection-item-lifecycle`). It persists a private decision before same-lane atomic removal, scans both lanes after unlink, then writes a content-free tombstone v2 and absence receipt. The receipt records a local absence observation, not owner identity verification or independent proof of every deletion step. Synthetic fixtures only; actual user data, approved records, training, provider calls, deployment, and production claims remain absent.
- The private collection item review projection accepts one live exact F1.10 item and writes only a content-free owner-only `pending-owner-review` projection (`npm run smoke:fine-tuning-private-collection-item-review-projection`). The reviewed-example lane records a required but unevaluated approved-training-record gate; the answer-quality lane records only an unsatisfied enrichment contract and missing Q1 field names. Current-chain, expiry, deletion, terminal, pending, removal, and history conflicts fail closed under the shared workspace lock. It does not copy item content or paths, create an approved record or case, or authorize training, provider use, submission, deployment, or production readiness.
- The private collection item review resolution binds one exact F1.12 final projection to a content-free quality-reviewer approve or reject decision (`npm run smoke:fine-tuning-private-collection-item-review-resolution`). It records only token and evidence hashes, checks the current F1 chain and F1.10 item under the shared workspace lock, resumes only known-valid pending history, and atomically publishes one decision/resolution bundle. Approval allows only the next lane-specific canonicalization or enrichment request; it creates no approved record or case and does not authorize candidate review, training, provider use, submission, deployment, or production readiness.
- The private collection item artifact request accepts only one exact approved F1.13 resolution (`npm run smoke:fine-tuning-private-collection-item-artifact-request`). It records lane-specific preparation intent and hashes without copying content or paths. It does not create an approved training record or answer-quality case, evaluate eligibility or Q1 content, or authorize candidate review, training, provider use, submission, deployment, or production readiness.
- The private collection item artifact preparation resolution binds one exact F1.14 request to a content-free quality-reviewer approve or reject decision (`npm run smoke:fine-tuning-private-collection-item-artifact-preparation-resolution`). Approval enables only the matching canonicalization or enrichment preparation lane; it never copies content, creates a record or case, or authorizes candidate review, training, provider use, submission, deployment, or production readiness.
- The private answer-quality enrichment candidate accepts only one live approved F1.15 `answer-quality-cases` resolution and curated-synthetic item (`npm run smoke:fine-tuning-private-answer-quality-enrichment-candidate`). It binds exact lineage references, hashes, lifecycle timestamps, and deterministic precheck metrics without storing objective, answer, sources, terms, paths, or the input filename. `q1ContractSatisfied: false` remains until separate reviewer review; no case, training, provider use, submission, deployment, or production authority is created.
- The private answer-quality enrichment candidate review resolution binds one canonical F1.16 final candidate to a content-free quality-reviewer approve or reject bundle (`npm run smoke:fine-tuning-private-answer-quality-enrichment-candidate-review-resolution`). Approval records only reviewer pass, `q1ReviewerGateSatisfied: true`, and an F1.18 materialization allowance. It keeps `q1ContractSatisfied`, case creation, case evaluation, training, provider use, submission, deployment, and production claims false. Reviewer identity and evidence remain independently unverified, and no actual content or payload is stored.
- The private answer-quality case materialization protocol accepts only the canonical approved F1.17 resolution, live F1.16 candidate, and owner-only enrichment input (`npm run smoke:fine-tuning-private-answer-quality-case`). It rebuilds retrieval and runs the fixed local Q1 evaluator before publishing one content-free logical case. The F1.18 record stores only hashes, counts, and metrics and grants no payload-storage authority.
- The private answer-quality case payload lifecycle requires a separate retention-owner decision before reading the enrichment input (`npm run smoke:fine-tuning-private-answer-quality-case-payload`). Approve stores only a curated-synthetic objective and the exact F1.18 case definition for local replay; reject stores no payload and does not read the raw input. The existing F1.11 command remains the only public deletion path and removes F1.19, F1.20, and then F1.18 through F1.16 before the item terminal bundle (`npm run smoke:fine-tuning-private-answer-quality-case-payload-lifecycle`). The receipt covers only repository-managed paths. Actual user data, training, provider use, submission, deployment, independent deletion proof, and production readiness remain unverified or absent.
- The private answer-quality readiness impact shadow revalidates one final F1.20 replay, rebuilds the tracked deterministic baseline, and adds its case definition to the suite only in memory (`npm run smoke:fine-tuning-private-answer-quality-readiness-impact`). It projects 2 to 3 answer-quality cases while dataset, split, export digests, collection completion, training authority, provider use, submission, deployment, and production readiness remain unchanged or false.
- The private combined readiness impact shadow accepts one final F1.21 reviewed record before adding one final F1.20 replay case in memory (`npm run smoke:fine-tuning-private-combined-readiness-impact`). The synthetic projection is 4/3/1/4/2 to 5/4/1/5/3; tracked state, training, provider calls, submission, deployment, and production readiness remain unchanged or false.
- The private collection-gap replan shadow independently rebuilds the F1.24 projection from trusted live authority, then recalculates the F1.2 collection plan only in memory (`npm run smoke:fine-tuning-private-collection-gap-replan`). The projected gaps are 15 reviewed examples, 5 mission scopes, and 7 answer-quality cases; tracked F1.2 evidence, intake, collection, training, provider, deployment, and production boundaries remain unchanged or false.
- A separate local-operator replay request can re-evaluate only the stored F1.19 curated-synthetic payload with the frozen F1.18 Q1 evaluator (`npm run smoke:fine-tuning-private-answer-quality-case-replay`). Its owner-only receipt is content-free and records no model call, training, provider use, submission, deployment, or production claim; it never reads the enrichment input.
