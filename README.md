...
- browser smoke는 temp workspace에 deterministic handoff fixture를 먼저 심은 뒤 최소 `index.md`/`index.txt`/`index.json` card가 실제 `ready` badge, non-empty path, `/api/execution-v1/handoff-artifacts/:id` direct-open href, `200 + expected content-type` 응답, release 탭 안의 inline preview 본문, `rartifact=index-markdown` reload restore, card-level/current-preview link copy의 `rartifact` deep-link와 `복사됨` label transition, 그리고 copied release preview deep-link를 새 browser session에서 다시 열었을 때 같은 preview panel이 복원되는지까지 모두 만족하는지 같이 잠금
